...
$page_access = 'MTSS Counselor Designee Principal Dept Head Superintendent Admin';
include('header.php');
...
if (isset($_POST['action'])) {
	...
	if ($_POST['testNum'] > 0) {
		for ($i = 1; $i < $_POST['testNum'] + 1; $i++) {
			...
			$query = "INSERT INTO interventions_scores (student_id, date, type, score) VALUES ($student_id, '$date', '$type', $score)";
			mysqli_query($dbc, $query) or die(json_encode(array('status' => 'fail', 'message' => mysqli_error($dbc), 'query' => $query)));
		}
	}
