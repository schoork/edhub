<?php
$page_title = 'DHA Expenditures Report';
$page_access = 'Admin Dept Head Principal Superintendent Designee';
include('header.php');
...
//Reimbursements
$query = "SELECT form_id, program, purchase_code, sum(total), program AS description, submit_datetime, status FROM forms_reimbursement AS fr LEFT JOIN forms AS f ON (fr.formId = f.form_id) WHERE purchase_code IS NOT NULL AND purchase_code NOT LIKE '%--%' AND status NOT LIKE 'Denied%' AND purchase_code LIKE '%-0001' ORDER BY purchase_code, form_id";
$result = mysqli_query($dbc, $query);
if (mysqli_num_rows($result) > 0) {
  while ($row = mysqli_fetch_array($result)) {
    if ($row['form_id'] != '') {
...
//Bus Requests
$query = "SELECT purchase_code, sum(total) AS total, title, submit_datetime, program, status, form_id FROM forms_busrequest AS fr LEFT JOIN forms AS f ON (fr.formId = f.form_id) WHERE purchase_code IS NOT NULL AND purchase_code NOT LIKE '%--%' AND status NOT LIKE 'Denied%' AND purchase_code LIKE '%-0001' GROUP BY purchase_code ORDER BY purchase_code";
$result = mysqli_query($dbc, $query);
if (mysqli_num_rows($result) > 0) {
  while ($row = mysqli_fetch_array($result)) {
    if ($row['form_id'] != '') {
  ...
  <div class="row">
    ...
    <table class="table-striped mt-3 mb-1">
      <tbody>
        ...
        echo '<tr><th colspan="4" style="text-align: right;">' . $fund . ' Total:</th><th style="text-align: right;">$' . number_format($total, 2) . '</th></tr>';
        echo '<tr><th colspan="4" style="text-align: right;">District Total:</th><th style="text-align: right;">$' . number_format($stotal, 2) . '</th></tr>';
        ?>
      </tbody>
    </table>
  </div>
