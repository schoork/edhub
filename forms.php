	...
	<div class="row">
      ...
      <p>
        Click on a form to view, deny, and/or approve it.
			...
			if ($_SESSION['departments'] !== '' && $_SESSION['departments'] !== null) {
				...
				<?php
				$query = "SELECT form_id, type, program, firstname, lastname, po, f.status, vendor, submit_datetime, superintendent, accounts_payable, transportation, business, payroll, federal_programs, sped, athletics, simmons, sanders, food_services FROM forms AS f LEFT JOIN staff_list AS sl ON (f.employee = sl.username) LEFT JOIN forms_requisition AS fr ON (f.form_id = fr.formId) WHERE f.status NOT LIKE 'Denied%' AND (";
				...
				if (mysqli_num_rows($result) > 0) {
					?>
					<p>
						<a class="btn btn-outline-primary" id="viewApproved" href="#!">View/Hide Partially Approved Forms</a>
					</p>
					<div class="alert alert-info">
						Fully approved forms have been moved to the reports section.
					</div>
						...
						<tbody>
							...
							while ($row = mysqli_fetch_array($result)) {
								...
								foreach ($depts as $dept) {
									if ($row[$dept] == $name || $row['status'] == 'Denied' || substr($row['status'], 0, 6) == 'Denied' || ($row['status'] == 'Approved' && $row['po'] <> '') || ($row['status'] == 'Approved' && $row['type'] != 'Requisition')) {
										echo ' display: none;" class="hidden';
										break;
									}
								}
								echo '" id="form-' . $row['form_id'] . '"><td>' . $row['form_id'] . '</td><td>' . $row['type'] . '</td><td>';
								if ($row['type'] == 'Requisition') {
									echo $row['vendor'];
								}
								else if ($row['type'] == 'Time Off Request') {
