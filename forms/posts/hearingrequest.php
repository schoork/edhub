...
if (!empty($_POST)) {
    ...
    $query = "INSERT INTO hearings (student_id, username, behavior, parent, phone, incident_date, student_date, suspended_date, parent_date, recommendation, num_days, final_date)";
    $query .= " VALUES ($student_id, '$username', '$behavior', '$parent', '$phone', '$incident_date', '$student_date', '$suspended_date', '$parent_date', '$recommendation', $num_days, '$final_date')";
    echo $query;
    ...
    $target_dir = "hearings/";
    //Student Statement
    $file_name = $_FILES["student_statement"]['name'];
    ...
    if (!move_uploaded_file($file_tmp, $target_dir . $file_name)) {
        ...
        $query = "INSERT INTO hearings_docs (hearing_id, type, name) VALUES ($formId, 'Student Statement', '$file_name')";
        mysqli_query($dbc, $query);
    }
    //Discipline Record
    $file_name = $_FILES["discipline"]['name'];
    while (file_exists($target_dir . basename($file_name))) {
        $file_name = "$formId " . $file_name;
    }
    $target_file = $target_dir . basename($file_name);
    if (!move_uploaded_file($file_tmp, $target_dir . $file_name)) {
        ...
        $query = "INSERT INTO hearings_docs (hearing_id, type, name) VALUES ($formId, 'Discipline Record', '$file_name')";
        mysqli_query($dbc, $query);
    }
    //Attendance Record
    $file_name = $_FILES["attendance"]['name'];
    while (file_exists($target_dir . basename($file_name))) {
        $file_name = "$formId " . $file_name;
    }
    $target_file = $target_dir . basename($file_name);
    if (!move_uploaded_file($file_tmp, $target_dir . $file_name)) {
        ...
        $query = "INSERT INTO hearings_docs (hearing_id, type, name) VALUES ($formId, 'Attendance Record', '$file_name')";
        mysqli_query($dbc, $query);
    }
    //Grades Record
    $file_name = $_FILES["grades"]['name'];
    while (file_exists($target_dir . basename($file_name))) {
        $file_name = "$formId " . $file_name;
    }
    $target_file = $target_dir . basename($file_name);
    if (!move_uploaded_file($file_tmp, $target_dir . $file_name)) {
        ...
        $query = "INSERT INTO hearings_docs (hearing_id, type, name) VALUES ($formId, 'Grades', '$file_name')";
        mysqli_query($dbc, $query);
    }
    //Counselor Referral
    $file_name = $_FILES["counselor"]['name'];
    while (file_exists($target_dir . basename($file_name))) {
        $file_name = "$formId " . $file_name;
    }
    $target_file = $target_dir . basename($file_name);
    if (!move_uploaded_file($file_tmp, $target_dir . $file_name)) {
        ...
        $query = "INSERT INTO hearings_docs (hearing_id, type, name) VALUES ($formId, 'Counselor Referral', '$file_name')";
        mysqli_query($dbc, $query);
    }
    //Optional Statements
    for ($i = 1; $i < 4; $i++) {
        $file_name = $_FILES["statement-$i"]['name'];
        if ($file_name != '') {
            while (file_exists($target_dir . basename($file_name))) {
                $file_name = "$formId " . $file_name;
            }
            ...
            if (!move_uploaded_file($file_tmp, $target_dir . $file_name)) {
                ...
                $query = "INSERT INTO hearings_docs (hearing_id, type, name) VALUES ($formId, 'Statement $i', '$file_name')";
                mysqli_query($dbc, $query);
            }
        }
    }
