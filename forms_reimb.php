...
$page_title = 'Reimbursements Report';
...
<div class="container mt-5 mb-5">
	<div class="row">
      ...
      <input type="hidden" id="form_type" value="reimbursement">
      <p class="lead">
        In the table below you will see all approved reimbursement requests.
      </p>
      <p>
		...
		$query = "SELECT * FROM forms AS f LEFT JOIN staff_list AS sl ON (f.employee = sl.username) WHERE f.status NOT LIKE 'Denied%' AND type = 'Reimbursement' ORDER BY submit_datetime DESC";
				...
				<table class="table table-striped dataTbl" id="tblApprove">
					...
					<tbody>
						...
						while ($row = mysqli_fetch_array($result)) {
							echo '<tr>';
							echo '<td><a class="btn btn-secondary" href="forms/prints/reimbursement.php?formId=' . $row['form_id'] . '" target="_blank">View</a></td>';
